...
import com.xjl.app.login.login.steps.LoginByPwdFragment;
import com.xjl.app.login.login.steps.LoginBySmsFragment;
...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        loginBySmsFragment.setCallback(new LoginBySmsFragment.Callback() {
            ...
            public void LoginBySmsCallback(User user) {
                LoginInfo.currentLoginUser = user;
                RXResult.setResult(LoginActivity.this, Activity.RESULT_OK,
                        IntentBuilder.newInstance().putExtra("user", user).build());
                finish();
            }
        });
    ...
    public static Observable<ActivityResult> loginState(AppCompatActivity activity) {
        if (LoginInfo.isLogin()) {
            ActivityResultFragment.insertActivityResult(activity, new ActivityResult(REQUEST_CODE_LOGIN));
        } else {
    ...
    public static ObservableTransformer<Bundle, Bundle> ensureLogin(final AppCompatActivity activity, int requestCode) {
        return upstream -> {
            upstream.subscribe(it -> {
                if (LoginInfo.isLogin()) {
                    RXResult.insertActivityResult(activity,
                            new ActivityResult(REQUEST_LOGIN, Activity.RESULT_OK,
                                    IntentBuilder.newInstance()
                                            .putExtra("user", LoginInfo.currentLoginUser).build())
                    );
                } else {
            ...
            return RXResult.obtainActivityResult(activity)
                    .filter(result -> result.requestCode == REQUEST_LOGIN)
